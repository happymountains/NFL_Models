...
use_python(
    "C:\\Users\\frobe\\AppData\\Local\\Programs\\Python\\Python39\\python.exe",
    required = T)
py_config()
source_python("nfl_schedule.py")

get_recent_weight <- function(week, num_weeks) {
    return(0.09)
}

get_recent_grade_weight <- function(week) {
    return(0.15)
}
...
get_pr <- function(years_data, teams, season, week, matchups) {
    ...
    last_weeks <- get_num_weeks(season - 1)

    total_frame <- data.frame(matrix(ncol = 3, nrow = 0))
    colnames(total_frame) <- c("Team", "O Total", "D Total")
    ...
    i <- week
    pyth_file <- read.csv(str_interp(
        "aEPA_Tests/season/overall/${season - 1}/Power_${season - 1}_week_${last_weeks}.csv"))
    if (i == 1) {
    ...
    dpepa_hash <- get_dpepa_values(teams, now_data, last_data,
        i, num_weeks, now_weight, last_weight)
    print(str_interp("--- Getting Defensive Rush EPA values ---"))
    drepa_hash <- get_drepa_values(teams, now_data, last_data,
        i, num_weeks, now_weight, last_weight)
    print(str_interp("--- Getting DPSR ---"))
    dpsr_hash <- get_dpsr_values(teams, now_data, last_data,
        i, num_weeks, now_weight, last_weight)
    print(str_interp("--- Getting DRSR ---"))
    drsr_hash <- get_drsr_values(teams, now_data, last_data,
        i, num_weeks, now_weight, last_weight)

    print(str_interp("--- Getting Offensive Pass EPA values ---"))
    opepa_hash <- get_opepa_values(teams, now_data, last_data,
        i, num_weeks, now_weight, last_weight)
    print(str_interp("--- Getting Offensive Rush EPA values ---"))
    orepa_hash <- get_orepa_values(teams, now_data, last_data,
        i, num_weeks, now_weight, last_weight)
    print(str_interp("--- Getting Offensive Pass Success Rate values ---"))
    opsr_hash <- get_opsr_values(teams, now_data, last_data,
        i, num_weeks, now_weight, last_weight)
    print(str_interp("--- Getting Offensive Run Success Rate values ---"))
    orsr_hash <- get_orsr_values(teams, now_data, last_data,
        i, num_weeks, now_weight, last_weight)
    
    print("--- Loading pythagorean values ---")
    pyth_vals <- get_pyth_values(teams, now_data,
        last_data, i, last_weeks, now_weight, last_weight, byes)

    for (team in teams) {
        log <- FALSE#team == "BAL"
        def_result <- get_adjusted_def_game(now_data, team, season,
            i, num_weeks, dpepa_hash, drepa_hash, dpsr_hash, drsr_hash)
        off_result <- get_adjusted_off_game(now_data, team, season,
        ...
        if (def_result == -1) {
            ...
            if (i == 1) {
                last_off <- now_offense[
                    now_offense$Team == team, "Off.Power"][[1]][[1]]
                last_off_noadj <- now_offense[
                    now_offense$Team == team, "Unadjusted"][[1]][[1]]
                last_def <- now_defense[
                    now_defense$Team == team, "Def.Power"][[1]][[1]]
                last_def_noadj <- now_defense[
                    now_defense$Team == team, "Unadjusted"][[1]][[1]]
                last_pyth_adj <- now_offense[
                    now_offense$Team == team, "PythAdj"][[1]][[1]]
            } else {
                last_off <- now_offense[
                    now_offense$Team == team, "Off.Power"][[1]][[1]]
                last_off_noadj <- now_offense[
            ...
            if (other_team != "") {
                other_def_result <- get_adjusted_def_game(now_data, other_team, season,
                    i, num_weeks, dpepa_hash, drepa_hash, dpsr_hash, drsr_hash)
                other_off_result <- get_adjusted_off_game(now_data, other_team, season,
                    i, num_weeks, opepa_hash, orepa_hash, opsr_hash, orsr_hash)
                
                team_pyth <- pyth_vals[[team]]
                other_pyth <- pyth_vals[[other_team]]

                off_adj <- off_result - 0.17 * as.double(now_defense[
                    now_defense$Team == other_team, "Def.Power"][[1]][[1]])
                def_adj <- def_result - 0.10 * as.double(now_offense[
                    now_offense$Team == other_team, "Off.Power"][[1]][[1]])
    ...
    final_frame <- data.frame(teams, def_ranking_vec, def_unadjusted, diff_vec, dpepa_vec, drepa_vec,
    ...
    final_frame <- data.frame(teams, off_ranking_vec, off_unadjusted, diff_vec, opepa_vec,
...
roll_up_grade <- function(filename, week) {
    ...
    if (old_grade == 0) {
        ...
        weight <- get_recent_grade_weight(week)
    }
...
roll_up_year_grade <- function(filename, week) {
    data <- read.csv(filename, strip.white=TRUE)
    i <- week
    total_grade <- 0
    bye <- 0
...
get_grade <- function(year, week, category) {
    ...
    for (i in 1:length(teams)) {
        team <- teams[i]
        current_grade <- roll_up_grade(str_interp(
            "aEPA_Tests/grades/${category}/${year}/${team}_${category}_${year}.csv"), week)
        last_grade <- roll_up_year_grade(str_interp(
            "aEPA_Tests/grades/${category}/${year - 1}/${team}_${category}_${year - 1}.csv"),
            get_num_weeks(year - 1))
        if (current_grade == -1) {
...
get_qb_value_nograde <- function(team, now_data, last_data, year, week, num_weeks) {
    ...
    for (team in teams) {
        ...
        qb_weight <- round(0.23 * week ^ 0.65, 3)
        if (NEW_QB[[team]] == TRUE) {
            qb_weight <- qb_weight + 0.05
        }
        ...
        if (nrow(team_recent ) == 0) {
            ...
            now_cpoe <- (team_season$cpoe * (1 - recent_weight) +
                team_recent$cpoe * (recent_weight)) * CPOE_REDUCTION
            now_epa <- team_season$epa * (1 - recent_weight) + team_recent$epa * (recent_weight)
        }
        ...
        final <- round(weigh_two_data(
            now_qb, last_qb, qb_weight, 1 - qb_weight, team), digits = 3)
        qb_vals[team] <- final
    }
...
get_category_value <- function(year, grades, week_weight, grade_weight) {
    temp_list <- list()
    temp_vals <- vector("double", length = 32)
    i <- 1
    for (team in teams) {
        last_grade <- as.double(grades[grades$Team == team, 'LastGrade']) * GRADE_REDUCTION
        new_grade <- as.double(grades[grades$Team == team, 'CurrentGrade']) * GRADE_REDUCTION
...
get_qb_value <- function(team, now_data, last_data, year, week, num_weeks,
    grades) {
    PCT_ONE <- 0.56
    PCT_TWO <- 0.40
    PCT_THREE <- 0.04
    ...
    # get this comparable to EPA (e.g. 90 --> 0.18)
    last_vals <- fetch_season_cpoe_vs_epa(last_data, num_weeks)
    ...
    qb_vals <- hash()
    for (team in teams) {
        team_grade <- as.double(grades[grades$Team == team, 'LastGrade']) * GRADE_REDUCTION
        team_last <- last_vals[which(last_vals$posteam == team),]
        team_recent <- recent_vals[which(recent_vals$posteam == team),]
        team_season <- season_vals[which(season_vals$posteam == team),]

        last_qb <- round(team_grade * PCT_ONE + team_last$epa * PCT_TWO +
            (team_last$cpoe * CPOE_REDUCTION) * PCT_THREE, digits = 3)
        ...
        now_qb <- round(new_grade * PCT_ONE + now_epa * PCT_TWO +
            now_cpoe * PCT_THREE, digits = 3)
...
get_pyth_values <- function(teams, now_data, last_data, week,
    ...
    for (team in teams) {
        ...
        if (!(week == 1 & byes[[team]] == 1)) {
            ...
            weighed_pyth <- weigh_two_data(pyth_now, pyth_last, now_weight,
                last_weight, team)
            weighed_wp <- weigh_two_data(wp_now, wp_last, now_weight,
                last_weight, team)
            pyth_values[[team]] <- round(weighed_pyth * 0.69 + weighed_wp * 0.31, 4)
        }
    }
...
get_dpepa_values <- function(teams, now_data, last_data, week,
    ...
    for (team in teams) {
        ...
        if (is.null(dpepa_now)) {
            ...
            recent_weight <- get_recent_weight(week, num_weeks)
            dpepa_now <- dpepa_recent * recent_weight + dpepa_now * (1 - recent_weight)
        }

        weighed_dpepa <- weigh_two_data(dpepa_now, dpepa_last, now_weight,
            last_weight, team)
...
get_drepa_values <- function(teams, now_data, last_data, week,
    num_weeks, now_weight, last_weight) {
    drepa_values <- hash()
    ...
    for (team in teams) {
        ...
        weighed_drepa <- weigh_two_data(drepa_now, drepa_last, now_weight,
            last_weight, team)
...
get_dpsr_values <- function(teams, now_data, last_data, week,
    num_weeks, now_weight, last_weight) {
    dpsr_values <- hash()
    ...
    for (team in teams) {
        ...
        weighed_dpsr <- weigh_two_data(dpsr_now, dpsr_last, now_weight,
            last_weight, team)
...
get_drsr_values <- function(teams, now_data, last_data, week,
    num_weeks, now_weight, last_weight) {
    drsr_values <- hash()
    ...
    for (team in teams) {
        ...
        weighed_drsr <- weigh_two_data(drsr_now, drsr_last, now_weight,
            last_weight, team)
...
get_opepa_values <- function(teams, now_data, last_data, week,
    num_weeks, now_weight, last_weight) {
    opepa_values <- hash()
    ...
    for (team in teams) {
        ...
        weighed_opepa <- weigh_two_data(opepa_now, opepa_last,
            now_weight, last_weight, team)
...
get_orepa_values <- function(teams, now_data, last_data, week,
    num_weeks, now_weight, last_weight) {
    orepa_values <- hash()
    ...
    for (team in teams) {
        ...
        weighed_orepa <- weigh_two_data(orepa_now, orepa_last,
            now_weight, last_weight, team)
...
get_opsr_values <- function(teams, now_data, last_data, week,
    num_weeks, now_weight, last_weight) {
    opsr_values <- hash()
    ...
    for (team in teams) {
        ...
        weighed_opsr <- weigh_two_data(opsr_now, opsr_last, now_weight,
            last_weight, team)
...
get_orsr_values <- function(teams, now_data, last_data, week,
    num_weeks, now_weight, last_weight) {
    orsr_values <- hash()
    ...
    for (team in teams) {
        ...
        weighed_orsr <- weigh_two_data(orsr_now, orsr_last, now_weight,
            last_weight, team)
...
write_power_rank_files <- function(year, years_data, week, byes) {
    ...
    total_frame <- data.frame(matrix(ncol = 3, nrow = 0))
    colnames(total_frame) <- c("Team", "OffTotal", "DefTotal")
    ...
    last_offense <- read.csv(str_interp(
        "aEPA_Tests/season/offense/${last_year}/Off_Ranking_${last_year}_${last_weeks}.csv"))
    last_defense <- read.csv(str_interp(
        "aEPA_Tests/season/defense/${last_year}/Def_Ranking_${last_year}_${last_weeks}.csv"))

    now_weight <- sides_weight(j, num_weeks)
    last_weight <- (1 - now_weight)

    if (TRUE) {
        grades <- get_grade(year, j, PASS_STR)
        qb_vals <- get_qb_value(team, years_data[[toString(year)]],
            years_data[[toString(last_year)]], year, j, last_weeks,
            grades)
    } else {
        qb_vals <- get_qb_value_nograde(team, years_data[[toString(year)]],
            years_data[[toString(last_year)]], year, j, last_weeks)
    }

    off_grades <- get_grade(year, j, OFF_STR)
    off_vals <- get_category_value(year, off_grades, test_weight(j), OFF_GRADE_PCT)
    pblk_grades <- get_grade(year, j, PBLK_STR)
    pblk_vals <- get_category_value(year, pblk_grades, test_weight(j), PBLK_PCT)
    recv_grades <- get_grade(year, j, RECV_STR)
    recv_vals <- get_category_value(year, recv_grades, test_weight(j), RECV_PCT)
    run_grades <- get_grade(year, j, RUN_STR)
    run_vals <- get_category_value(year, run_grades, test_weight(j), RUN_PCT)
    rblk_grades <- get_grade(year, j, RBLK_STR)
    rblk_vals <- get_category_value(year, rblk_grades, test_weight(j), RBLK_PCT)
    def_grades <- get_grade(year, j, DEF_STR)
    def_vals <- get_category_value(year, def_grades, test_weight(j), DEF_GRADE_PCT)
    rdef_grades <- get_grade(year, j, RDEF_STR)
    rdef_vals <- get_category_value(year, rdef_grades, test_weight(j), RDEF_PCT)
    tack_grades <- get_grade(year, j, TACK_STR)
    tack_vals <- get_category_value(year, tack_grades, test_weight(j), TACK_PCT)
    prsh_grades <- get_grade(year, j, PRSH_STR)
    prsh_vals <- get_category_value(year, prsh_grades, test_weight(j), PRSH_PCT)
    cov_grades <- get_grade(year, j, COV_STR)
    cov_vals <- get_category_value(year, cov_grades, test_weight(j), COV_PCT)
    ...
    for (k in 1:length(teams)) {
        ...
        now_offense_value <- now_offense[
            now_offense$Team == team, "Off.Power"][[1]][[1]]
        last_offense_value <- last_offense[
            last_offense$Team == team, "Off.Power"][[1]][[1]]
        now_defense_value <- now_defense[
            now_defense$Team == team, "Def.Power"][[1]][[1]]
        last_defense_value <- last_defense[
            last_defense$Team == team, "Def.Power"][[1]][[1]]
        now_pyth_value <- now_offense[
            now_offense$Team == team, "Pyth"][[1]][[1]]
        now_pyth_value <- now_pyth_value * 0.7 + now_offense[
            now_offense$Team == team, "PythAdj"][[1]][[1]] * 0.3
        ...
        row <- c(team, round(overall, 3), round(now_pyth_value, 3), round(offense, 3),
    ...
    for (index in 1:length(report_frame$PowerRating)) {
        ...
        report_frame$PowerRating[index] <- round(PR_MULT * 
            (as.double(report_frame$PowerRating[index]) - pr_med) + off_vals[[team]] +
            pblk_vals[[team]] + recv_vals[[team]] + run_vals[[team]] + rblk_vals[[team]] +
            def_vals[[team]] + rdef_vals[[team]] + tack_vals[[team]] + 
            prsh_vals[[team]] + cov_vals[[team]], 2)
        report_frame$Pyth[index] <- round((PR_MULT * 
            (as.double(report_frame$Pyth[index]) - pyth_med) * PYTH_PCT), 2)
        report_frame$Off[index] <- round((PR_MULT * 
            (as.double(report_frame$Off[index]) - off_med) * OFF_PCT) + off_vals[[team]] +
            pblk_vals[[team]] + recv_vals[[team]] + run_vals[[team]] + rblk_vals[[team]], 2)
        report_frame$Def[index] <- round((PR_MULT * 
            (as.double(report_frame$Def[index]) - def_med) * DEF_PCT * -1) + def_vals[[team]] +
            rdef_vals[[team]] + tack_vals[[team]] + 
            prsh_vals[[team]] + cov_vals[[team]], 2)
        report_frame$QB[index] <- round((PR_MULT * 
            (as.double(report_frame$QB[index]) - qb_med) * QB_PCT), 2)

        team <- (report_frame$Team[index])
        offense <- as.double(report_frame$Off[index])
        defense <- as.double(report_frame$Def[index])
        qb <- as.double(report_frame$QB[index])

        off_total <- (points + 2 * qb +  1.2 * offense)
        def_total <- (points - 1.3 * defense)
        row <- c(team, round(off_total, 1), round(def_total, 1))
...
get_lines <- function(season, week, matchups) {
    ...
    for (i in 1:length(matchups)) {
        ...
        away_rating <- as.double(epa_frame[
            epa_frame$Team == away, "PowerRating"][[1]][[1]])
        home_rating <- as.double(epa_frame[
            epa_frame$Team == home, "PowerRating"][[1]][[1]])
...
sides_weight <- function(x, num_weeks) {
    ...
    # val <- round(1.19 - 1.19*exp(1)^(-0.18*x), 3) # 55.3
...
get_average_ppg <- function(season, week, teams) {
    ...
    for (i in 0:WEEKS_SUMMED) {
        ...
        total <- total + get_week_average_points(years_data[[
            toString(current_year)]], current_year, current_week, teams)
    }
...
get_totals <- function(season, week, matchups) {
    ...
    total_frame <- data.frame(matrix(ncol = 3, nrow = 0))
    colnames(total_frame) <- c("Away", "Total", "Home")
    ...
    for (i in 1:length(matchups)) {
        ...
        away_half <- round(0.61 * away_o + 0.39 * home_d, 2)
        home_half <- round(0.61 * home_o + 0.39 * away_d, 2)
        total <- round(away_half + home_half, 1)
        total_frame[nrow(total_frame) + 1,] <- c(away, total, home)
    }
...
get_game_sides <- function(years_data) {
    ...
    colnames(report_frame) <- c("Season", "Week", "Away", "Home", "Result",
            ...
            for (i in 1:nrow(picks)) {
                ...
                if (score_diff >= VALUE & score_diff < 5.3) {
                    result <- tryCatch({
                            ...
                            report_frame[num_rows,] <- c(season, week, away_team,
...
get_total_results <- function(years_data) {
    ...
    print(str_interp("Record: ${w_l}"))
    print(str_interp("Win Percentage: ${wp}"))
    print(str_interp("Total Profit: ${profit}"))
...
get_average_total <- function(years_data, year, nfl_week) {
    ...
    if (nfl_week > 1) {
        now_average <- years_data[[toString(year)]] %>% 
            ...
            group_by(game_id) %>%
            slice(n())
        total <- mean(now_average$total_line)
    ...
    old_average <- years_data[[toString(year - 2)]] %>% 
        ...
        group_by(game_id) %>%
        slice(n())
    total <- total + mean(old_average$total_line)
    ...
    last_average <- years_data[[toString(year - 1)]] %>% 
        ...
        group_by(game_id) %>%
        slice(n())
    total <- total + mean(last_average$total_line)
...
# matchups <- get_schedule(2023, 1)
# get_lines(2023, 1, matchups)
# get_totals(2023, 1, matchups)

year <- 2023
num_weeks <- get_num_weeks(year)
for (a_week in CURRENT_WEEK:CURRENT_WEEK) {
    matchups <- get_schedule(year, a_week)
    get_pr(years_data, teams, year, a_week, matchups)
    write_power_rank_files(year, years_data, a_week,
        get_team_byes(year))
    matchups <- get_schedule(year, a_week + 1)
    get_lines(year, a_week + 1, matchups)
    get_totals(year, a_week + 1, matchups)
}
